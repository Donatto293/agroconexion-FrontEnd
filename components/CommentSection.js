...
const CommentSection = ({ productId, user, onAuthRequired }) => {
  ...
  try {
    ...
    if (status !== 'granted') {
      Alert.alert('Permiso denegado', 'Necesitamos acceso a tus imágenes para seleccionar fotos.');
      return;
    }
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      aspect: [4, 3],
      quality: 0.8,
      // selectionLimit: 0, // opcional: en algunas versiones permite selección múltiple (0 = ilimitado)
    });
    ...
    // extraer URIs de forma segura
    let uris = [];
    if (Array.isArray(result.assets)) {
      // nuevo formato: result.assets => [{ uri, ... }, ...]
      uris = result.assets.map(a => a.uri || a.localUri).filter(Boolean);
    } else if (result.uri) {
      // formato antiguo
      uris = [result.uri];
    } else if (result.Image && result.Image.uri) {
      // por si algún plugin devuelve otro shape inesperado
      uris = [result.Image.uri];
    }
